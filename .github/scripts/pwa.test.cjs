...
let base, version = 'one', enabled = true, account = '';
...
const server = http.createServer((req, res) => {
  ...
  if (url.searchParams.has('rv_sw')) {
    ...
    res.end('self.RV_SW = ' + JSON.stringify({ version: 'test-1.7.1', dataPath: base + '/data/', uploads: '/uploads/', romExt: ['nes'], skip: ['/wp-login.php', '/account/', '/wp-json/'], precache: [], offline: '/offline/', maxPages: 60, maxShell: 300 }) + ';\n' + fs.readFileSync(path.join(assets, 'sw.js'))); return;
  }
